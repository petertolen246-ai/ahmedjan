...
do
    ...
    -- ── Запуск ───────────────────────────────────────────────────────────────
    -- 1) Пробуем сохранённый ключ
    local saved = fsRead(SAVE_FILE)
    if saved then
        saved = saved:gsub("%s+", ""):upper()
        local ok, msg, extra = verify(saved)
        if ok then
            print("[AhmedjanKS] ✅ Авто-вход | " .. (extra or ""))
            verified = true
        else
            warn("[AhmedjanKS] Сохранённый ключ невалиден: " .. msg)
            fsWrite(SAVE_FILE, "")
        end
    end
    ...
    if not verified then
        ...
        verifySignal.Event:Wait()
    end

    verifySignal:Destroy()
